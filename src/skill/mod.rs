...
pub use executor::{LuaSkillExecutor, ShellSkillExecutor, SkillExecutor};
...
mod tests {
    ...
    fn skill_execution_variants_serialize() {
        ...
        let lua = SkillExecution::Lua {
            code: "return 42".into(),
            timeout_secs: Some(30),
        };
        let json = serde_json::to_string(&lua).unwrap();
        assert!(json.contains("\"type\":\"lua\""));
