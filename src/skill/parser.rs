...
fn parse_execution(raw: &RawExecution) -> SoulResult<SkillExecution> {
    match raw.exec_type.as_str() {
        ...
        "lua" => {
            let code = raw.code.clone().ok_or_else(|| SoulError::SkillParse {
                message: "Lua execution requires 'code'".into(),
            })?;
            Ok(SkillExecution::Lua {
                code,
                timeout_secs: raw.timeout_secs,
            })
        }
...
"#;
    ...
    const LUA_SKILL: &str = r#"---
name: api_query
description: Query an API with Lua logic
input_schema:
  type: object
  properties:
    endpoint:
...
"#;
    ...
    #[test]
    fn parse_lua_skill() {
        let skill = parse_skill(LUA_SKILL).unwrap();
        assert_eq!(skill.name, "api_query");
        assert_eq!(skill.description, "Query an API with Lua logic");
        assert!(skill.body.contains("Lua-powered"));
        match &skill.execution {
            SkillExecution::Lua { code, timeout_secs } => {
                assert!(code.contains("fetch(url)"));
                assert!(code.contains("url_encode"));
                assert_eq!(*timeout_secs, Some(30));
            }
            _ => panic!("Expected Lua execution"),
        }
    }

    #[test]
    fn lua_missing_code_errors() {
        let content = r#"---
name: bad_lua
