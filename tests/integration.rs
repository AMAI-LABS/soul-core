...
async fn event_stream_completeness() {
    ...
    let event_types: Vec<String> = events
        ...
        .map(|e| match e {
            ...
            AgentEvent::StructuralRetry { .. } => "structural_retry",
            AgentEvent::SubagentStart { .. } => "subagent_start",
            AgentEvent::SubagentEnd { .. } => "subagent_end",
            AgentEvent::PlanExecutionStart { .. } => "plan_execution_start",
            AgentEvent::PlanTaskStart { .. } => "plan_task_start",
            AgentEvent::PlanTaskEnd { .. } => "plan_task_end",
            AgentEvent::PlanExecutionEnd { .. } => "plan_execution_end",
        })
        .map(|s| s.to_string())
        .collect();
